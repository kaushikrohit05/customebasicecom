...
class PS_Pagination {
	...
	function paginate() {
		...
		$all_rs = @$db->runQuery($this->sql );
		if (!$all_rs) {
			if ($this->debug)
				echo "SQL query failed. Check your query.<br /><br />Error Returned: ";
			 	return false;
		 }
		...
		$rs = @$db->runQuery($this->sql . " LIMIT {$this->offset}, {$this->rows_per_page}" );
		if (! $rs) {
			if ($this->debug)
				echo "Pagination query failed. Check your query.<br /><br />Error Returned: ";
			return false;
		}
		return $rs;
	}
